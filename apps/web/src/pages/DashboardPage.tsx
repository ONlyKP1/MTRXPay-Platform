...
export function DashboardPage() {
  ...
  // Show locked state if merchant is not approved
  const merchantStatus = user?.merchantStatus ?? 'draft';
  if (merchantStatus !== 'approved') {
    return (
      <DashboardLayout unreadCount={0}>
        <DashboardLocked status={merchantStatus} firstName={user?.firstName} />
      </DashboardLayout>
    );
  }
