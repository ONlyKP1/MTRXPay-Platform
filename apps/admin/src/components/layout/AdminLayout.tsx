...
const icons = {
  ...
  transactions: (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
      <polyline points="17 1 21 5 17 9" /><line x1="3" y1="5" x2="21" y2="5" /><polyline points="7 23 3 19 7 15" /><line x1="21" y1="19" x2="3" y2="19" />
    </svg>
  ),
  settlements: (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
  ...
  risk: (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
      <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" /><path d="M12 8v4" /><circle cx="12" cy="16" r="0.5" fill="currentColor" />
    </svg>
  ),
  reports: (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
...
const navSections = [
  ...
  },
  {
    label: 'ONBOARDING',
    items: [
      { label: 'Applications', icon: icons.applications, to: '/applications' },
      { label: 'Merchants', icon: icons.merchants, to: '/merchants' },
      { label: 'KYC/KYB Reviews', icon: icons.reviews, to: '/reviews' },
      { label: 'Communications', icon: icons.communications, to: '/communications' },
    ],
  },
  {
    label: 'OPERATIONS',
    items: [
      ...
      { label: 'Settlements', icon: icons.settlements, to: '/settlements' },
      { label: 'Disputes', icon: icons.disputes, to: '/disputes' },
      { label: 'Reconciliation', icon: icons.reconciliation, to: '/reconciliation' },
      { label: 'Risk Monitor', icon: icons.risk, to: '/risk' },
    ],
  },
  {
    label: 'INTELLIGENCE',
    items: [
      ...
      { label: 'Audit Log', icon: icons.audit, to: '/audit-log' },
    ],
  },
  {
    label: 'PLATFORM',
    items: [
      ...
      { label: 'Dev Tools', icon: icons.developer, to: '/developer' },
    ],
  },
];
...
export function AdminLayout() {
  ...
  return (
    <div className="admin">
      <button className="admin__burger" onClick={() => setSidebarOpen(!sidebarOpen)} aria-label="Toggle menu">
        ...
        <div className="admin__sidebar-top">
          ...
          <nav className="admin__nav">
            {navSections.map((section) => (
              <div key={section.label} className="admin__nav-section">
                <span className="admin__nav-label">{section.label}</span>
                {section.items.map((item) => (
                  <Link
                    key={item.to}
                    to={item.to}
                    className={`admin__nav-item${location.pathname === item.to || (item.to !== '/' && location.pathname.startsWith(item.to)) ? ' admin__nav-item--active' : ''}`}
